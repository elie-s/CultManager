...
namespace CultManager
{
    public class Platform : MonoBehaviour
    {
        ...
        public GameObject SpawnCultist(GameObject _prefab, Transform _parent, Cultist _cultist, bool _spawnAtOrigin, int _orderInLayer)
        {
            Vector2 pos = _spawnAtOrigin ? (Vector2)transform.position : Evaluate(Random.value);
            GameObject gObject = Instantiate(_prefab, new Vector3(pos.x, pos.y, Random.value * 0.0001f), Quaternion.identity, _parent);
            gObject.GetComponent<CultistAnimationBehaviour>().Init(this, _cultist, _orderInLayer);
